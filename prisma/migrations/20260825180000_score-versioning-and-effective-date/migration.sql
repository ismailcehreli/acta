...
BEGIN
  ...
  IF is_frozen IS NULL THEN
    RAISE EXCEPTION 'SCORE_FACT_PERIOD_MISSING: the score fact has no revision'
      USING ERRCODE = '23514';
  END IF;

  IF is_frozen THEN
    RAISE EXCEPTION
      'SCORE_FACT_PERIOD_FROZEN: a score fact cannot be added to a frozen revision'
      USING ERRCODE = '23514';
  END IF;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    RAISE EXCEPTION
      'FROZEN_PERIOD_IMMUTABLE: a frozen score revision cannot be deleted'
      USING ERRCODE = '23514';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'FROZEN_PERIOD_IMMUTABLE: a frozen score revision cannot be changed'
      USING ERRCODE = '23514';
  END IF;
...
BEGIN
  ...
  effective_at := score_effective_at(
  ...
  INSERT INTO "ScoreUserStateEvent" (
    ...
    gen_random_uuid(), NEW."id", effective_at, NEW."isActive", NEW."isScored",
...
LANGUAGE plpgsql AS $$
DECLARE
  calendar_row "OrgUnitWorkCalendar"%ROWTYPE;
...
BEGIN
  ...
  INSERT INTO "ScoreUnitCalendarEvent" (
    ...
    gen_random_uuid(), calendar_row."orgUnitId", score_effective_at(CURRENT_TIMESTAMP),
    has_calendar, calendar_row."workingDays", calendar_row."workStartMinute", calendar_row."workEndMinute",
    calendar_row."worksOnHolidays", score_change_reason(), score_change_actor()
  );
...
LANGUAGE plpgsql AS $$
DECLARE
  holiday_row "Holiday"%ROWTYPE;
BEGIN
  holiday_row := CASE WHEN TG_OP = 'DELETE' THEN OLD ELSE NEW END;
  INSERT INTO "ScoreHolidayEvent" (
    "id", "holidayDate", "effectiveAt", "isHoliday", "description",
    "reason", "actorId"
  ) VALUES (
    gen_random_uuid(), holiday_row."date", score_effective_at(CURRENT_TIMESTAMP),
    TG_OP <> 'DELETE', holiday_row."description", score_change_reason(),
    score_change_actor()
  );
  RETURN CASE WHEN TG_OP = 'DELETE' THEN OLD ELSE NEW END;
END;
...
LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP = 'DELETE' AND current_setting('app.demo_purge', true) = 'yes' THEN
    RETURN OLD;
  END IF;

  RAISE EXCEPTION 'SCORE_HISTORY_IMMUTABLE: score history cannot be changed'
    USING ERRCODE = '23514';
END;
$$;
