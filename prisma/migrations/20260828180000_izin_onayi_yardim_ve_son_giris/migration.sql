-- İzin talepleri, yardım kütüphanesi ve son başarılı giriş bilgisi.
...
-- Bekleyen ve onaylı kayıtlar geçerli aralıkta birbirleriyle çakışamaz.
-- Reddedilen veya iptal edilen kayıtlar yeni bir talebi engellemez.
...
LANGUAGE plpgsql AS $$
BEGIN
  -- Bekleyen bir talep, karar bilgisi taşıyamaz. Eski onaylı satırlarda
  -- karar bilgisi bulunmaması geriye dönük uyumluluk için korunur.
  IF NEW."status" = 'PENDING'
     AND (NEW."decidedAt" IS NOT NULL
       OR NEW."decidedById" IS NOT NULL
       OR NEW."decisionReason" IS NOT NULL) THEN
    RAISE EXCEPTION 'NO_ACTIVITY_PENDING_WITH_DECISION: bekleyen kayıt karar bilgisi taşıyamaz';
  END IF;
...
-- Yardım içeriği arşivlenir; fiziksel silme yalnız örnek veri temizliğinin
-- transaction'a özel kapısından geçebilir.
