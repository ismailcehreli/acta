...
function error(message: string): HelpFormState {
...
export async function createHelpArticleAction(
  ...
  if (!me) return error(t("screens.help.permissionCreate"));

  const parsed = helpArticleSchema.safeParse(articleValues(formData));
  if (!parsed.success) {
    return error(
      localizeValidationIssue(t, parsed.error.issues[0], "screens.help.invalid"),
    );
  }

  const result = await createHelpArticle(prisma, me.id, parsed.data);
  if (!result.ok) return error(localizeServiceMessage(t, "help", result));
...
export async function updateHelpArticleAction(
  ...
  const me = await requireHelpManager();
  const t = await getTranslations();
  if (!me) return error(t("screens.help.permissionEdit"));
  ...
  revalidatePath("/help");
  revalidatePath(`/help/${parsed.data.id}/edit`);
  redirect("/help");
}
